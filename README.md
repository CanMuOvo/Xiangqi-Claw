...
### 人机对战
- **四档难度**：入门 / 普通 / 困难 / 大师（不同搜索深度 + 概率选招，新手到高手都能找到合适的对手）
- 可自由选择电脑执红 / 执黑 / 双人对弈
- 长将规则：连续将军 5 次后禁止该着法，杜绝"一直跟着"的烂招

### 棋谱库
- 按**学习路径**分类：杀法（适情雅趣·卷一、四大名局）→ 中局（运子/兑换/谋子/弃子等战术）→ 布局（屏风马/顺炮/列炮/仙人指路/飞相等开局定式 + 布局陷阱）→ 残局（实用残局例胜例和定式）
- 全屏复盘界面：逐步播放（上一步/下一步/自动播放/进度条拖拽），同步显示**走棋记录**与**引擎实时分析**
- 点击任意一步可跳转并分析该局面

### 沙盘推演
- 自由控制红黑双方模拟走棋，随时回退/前进
- 引擎自动推演到终局，AI 教练的走法演示一键进入沙盘逐步播放

### 走棋提醒
- 开启后，每步与最优解差距超过 150 分时在棋盘中央提示「漏招」
- 适合对照学习：走完一步立即知道这步好不好

### 棋盘主题
- 多款经典主题：**经典木纹 / 翡翠丝绒（墨绿绒布）/ 乌檀鎏金（古风深色）**，棋盘子力、纹理、河界风格统一可换

### 终局展示
- 落子动画与绝杀特效播完后，棋盘中央弹出**胜负卡**：胜方棋子徽章 + 粒子爆开 + 渐变发光标题 +「再开一局」
- 随胜方配色（红胜朱砂 / 黑胜黛蓝 / 和棋鎏金）

### 其他功能
- **翻转棋盘** — 切换红方/黑方在下方的视角
- **走棋历史** — 完整的走棋记录，可点击回到任意历史局面
- **新局** — 一键重新开始
- **响应式布局** — PC 平滑压缩（700px 以上保持双栏）、手机端底部 Tab 导航；顶栏「棋盘大屏」开关可随时切换单栏全屏
- **棋谱库全屏界面** — 独立页面浏览复盘，返回时自动恢复原对局
